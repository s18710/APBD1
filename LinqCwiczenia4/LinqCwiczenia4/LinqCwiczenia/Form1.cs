...
namespace LinqCwiczenia
{
 ...
 //           var p6 = Emps.Sum(emp => emp.Salary);
        ...
        {
            int max = Emps.Max(emp => emp.Salary);

            var result = Emps.Where(emp => emp.Salary == max).ToList();
        ...
        {
            var result = Emps.GroupBy(emp => emp.Job).Select(lg => new
            {
                Praca = lg.Key ,
                LiczbaPracownikow = lg.Count()
            });
        ...
        {
            var result = Emps.Where(emp => emp.Job == "Frontend programmer").OrderByDescending(emp => emp.HireDate).FirstOrDefault();
        ...
        {
            var nullSet = { "Brak wartości", null, null };
            ...
            var union = result.Union(nullSet);
